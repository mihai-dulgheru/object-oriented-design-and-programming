...
public class SQLiteConnection {
    ...
    public static void main(String[] args) throws SQLException {
        ...
        try {
            ...
            int nr = 0;
            while (rs.next()) {
                System.out.println(++nr + " Nume " + rs.getString(2) + " CNP " + rs.getString(5));
                listaStud.add(new Student(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getInt(4), rs.getString(5),
            ...
            List<Student> listaStudZI = listaStud.stream().filter(s -> s.getForma().equals("ZI"))
                    .collect(Collectors.toList());
            for (Student s : listaStudZI)
                System.out.println(s.toString());

            System.out.println("---------------------inaltime > 178");
            List<Student> listaStudInaltime = listaStud.stream().filter(s -> s.getInaltime() >= 178)
                    .collect(Collectors.toList());
            ...
            String sqlUpdate1 = "UPDATE student SET NUME='" + listaStud.get(pozitie).getNume() + "' where COD="
                    + listaStud.get(pozitie).getCod();
